...
const AdminListings = () => {
  ...
  const [filter, setFilter] = useState('Pending'); // Start with Pending for admins
  ...
  const fetchListings = async () => {
    setLoading(true);
    try {
      // For now we'll fetch all or pending based on filter
      const endpoint = filter === 'Pending' ? '/api/admin/listings/pending' : '/api/listings';
      ...
      console.error("Failed to fetch listings for moderation:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredListings = adminListings.filter(l => {
    const title = (l.title || 'Untitled').toLowerCase();
  ...
  return (
    <div className="admin-page-container">
      ...
      <div className="admin-table-container" style={{ background: 'white', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
        {loading ? (
            <div style={{ padding: '40px', textAlign: 'center' }}>Loading listings...</div>
        ) : (
                ...
                <tr key={l._id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                  <td style={{ padding: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: '12px', color: '#717171' }}>ID: {String(l._id).slice(-6)}</div>
                      </div>
                    </div>
                  </td>
                  <td style={{ padding: '16px', color: '#444' }}>{l.location}</td>
                  <td style={{ padding: '16px', color: '#444' }}>{l.propertyType || l.type}</td>
                  <td style={{ padding: '16px' }}>
                    <span style={{ 
                      padding: '4px 10px', 
                      borderRadius: '12px', 
                  ...
                  <td style={{ padding: '16px', textAlign: 'right' }}>
                    <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                       <Link to={`/rooms/${l._id}`} title="View" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '32px', height: '32px', borderRadius: '50%', background: '#f0f0f0', color: '#222' }}>
                          <Eye size={16} />
                       </Link>
